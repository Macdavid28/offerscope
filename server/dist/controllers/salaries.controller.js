...
import { computeConfidenceScore, computeTotalCompensation, normalizeCompany, normalizeLevel, buildFilterQuery, enrichSalary, calculatePercentileValue, normalizeCompensation, getLevelScore, } from "../utils/salary.utils.js";
import { ingestSalarySchema } from "../schemas/salary.schema.js";
export const ingestSalary = async (req, res) => {
    // Validate using Zod
    const validatedData = ingestSalarySchema.parse(req.body);
    const { company, role, level, location, baseSalary, bonus, stock, experienceYears, currency, compensationPeriod, } = validatedData;
    // Normalization
...
export const getSalaries = async (req, res) => {
    ...
    const companyLevelPairs = Array.from(new Set(salaries.map((s) => `${s.company}|${s.levelStandardized}`))).map((pair) => {
    ...
    const enrichedSalaries = salaries.map((salary) => {
        const peers = peerData.filter((p) => p.company === salary.company &&
...
export const getCompanyStats = async (req, res) => {
    ...
    const levels = allSalaries.reduce((acc, curr) => {
    ...
    const levelStats = Object.keys(levels).reduce((acc, key) => {
    ...
    const enrichedPaginated = paginatedSalaries.map((s) => {
        const peers = allSalaries.filter((p) => p.levelStandardized === s.levelStandardized);
...
export const compareSalaries = async (req, res) => {
